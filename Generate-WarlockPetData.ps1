# Generates PetTips Data/<flavor>/DemonAbilitiesData.lua from the matching
# client DB (wago.tools CSVs; Vanilla = 1.15 era build, TBC = 2.5.6
# wow_anniversary build). Unlike hunter pets, warlock demons learn
...
# no trainable abilities and are excluded; Felguard: 2887 on Era is SoD-only
# and excluded, 29 on TBC is the 41-point Demonology talent and included)
...
# 'Intercept Stun' = the triggered stun sub-spells of Felguard Intercept
# (not learnable themselves); 32851 = an unused duplicate of Demonic
# Frenzy 32850 with no grimoire and no auto-learn (TBC 2.5.6 DB).
$excludeSpell = @{ 32851 = $true }
...
foreach ($r in (Import-Csv "$sp\SkillLineAbility.csv")) {
    ...
    $s = [int]$r.Spell
    if ($excludeSpell.ContainsKey($s)) { continue }
    $n = $nameById[$s]
    if (-not $n -or $n -match 'DND|Scaling|Demonic Pact|Intercept Stun|Effect$') { continue }
...
"-- Warlock demon abilities for ${flavorName}: every rank a demon can learn.$nl" +
...
"-- No training points, no taming. Spell IDs / rank order from the $clientVer$nl" +
